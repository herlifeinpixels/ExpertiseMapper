/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/1260*@* */

/* Expertise mapping of nodes */
//Technology
// - Communications
// - Design
// - Management
...
// work on collecting cata info for colorizing.

//import processing.xml.*;

String graphFile = "expertise50.xml";
float zoomFac = 4;
...
void addNext()
{
 ...
 int i = nbrAdded;
    ...
    int cata = kid.getIntAttribute("cata");
    int impor = kid.getIntAttribute("importance");
    ...
    particles[i].cata = cata;
    particles[i].impor = impor;
    
    // TODO add relationships and # times mentioned
    lastDepth = depth;
    if (depth == 0) {
...
*/
  ...
  for (int i = 0; i < springs.length; ++i)
  {
    ...
    if (spr == null)
      continue;
    color c1 = color((sin(spr.dot1.cata*gAngle)+1)/2,.5 + (sin(spr.dot1.depth*gAngle)+1)/4,.7);
    stroke(c1);
  ...
  for (int i = 0; i < particles.length; ++i) 
  {
    ...
    if (pi == null)
      continue;
    color c1 = color((sin(pi.cata*gAngle)+1)/2,.5 + (sin(pi.depth*gAngle)+1)/4,.9);
    fill(c1);
    strokeWeight(pi.impor); // TODO: Add stroke
    stroke(200);
...
void mousePressed()
{
  for (int i = 0; i < particles.length; ++i) 
  {
   ...
   {
     label = pi.name;
     drawCurrentText(label);     
     // TODO
     // Add focus to nodes in xml.getInt("rel")
     // Foreach i in rel { focus.append = xml.getChild(i);  }
     // Unfocus the remaining nodes
   }
  }
  // find nearest node, if we're sitting on one, show it's label...
}

void drawCurrentText(String label) {
  fill(200);
  textSize(24);
  text(label, 10, 10, 70, 80);
