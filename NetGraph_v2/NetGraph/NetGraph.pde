...
void addNext()
{
 ...
 int i = nbrAdded;
    ...
    int depth = kid.getInt("depth");
    int genre = kid.getInt("genre");
    String nam = kid.getString("name");
    int parentID = 0;
    ...
    if (depth > 0) {
      ...
      parentID = kid.getInt("parent");
      for (int j = 0; j < i; ++j) {
        if (xml.getChild(j).getInt("id") == parentID)
        {
          parent = particles[j];
...
void setup()
{
  ...
  xml = loadXML(graphFile);
  //nbrNodes = xml.getIntAttribute("nbrnodes");
  nbrNodes = xml.getInt("nbrnodes");
  println(nbrNodes + " nodes");
  //int numSites = xml.getChildCount();
  int numSites = xml.getChildren().length;
